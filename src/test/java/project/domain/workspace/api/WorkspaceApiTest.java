...
public class WorkspaceApiTest extends IntegrationTest {
    ...
    public void 워크스페이스_유저_조회() throws Exception {
        // given
        // when
        mvc.perform(get("/v1/workspaces/1/users").header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN_USER))
                // then
                .andExpect(status().isOk())
    ...
    public void 워크스페이스_유저_조회_외부인() throws Exception {
        // given
        // when
        mvc.perform(get("/v1/workspaces/1/users").header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN_OUTSIDER))
                // then
                .andExpect(status().isForbidden());
    }
    ...
    public void 워크스페이스_유저_추방() throws Exception {
        // given
        // when
        mvc.perform(delete("/v1/workspaces/1/users/2").header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN))
                // then
                .andExpect(status().isOk());
    ...
    public void 워크스페이스_유일_방장_추방() throws Exception {
        // given
        // when
        mvc.perform(delete("/v1/workspaces/1/users/1").header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN))
                // then
                .andExpect(status().isNotFound());
    }
    ...
    public void 워크스페이스_유저_추방_권한없음() throws Exception {
        // given
        // when
        mvc.perform(delete("/v1/workspaces/1/users/2").header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN_USER))
                // then
                .andExpect(status().isForbidden());
    }
