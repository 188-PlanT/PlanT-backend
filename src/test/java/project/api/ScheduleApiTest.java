...
public class ScheduleApiTest extends IntegrationTest {
    ...
    public void 스케줄_댓글_삭제() throws Exception {
        // given
        // when
        mvc.perform(delete("/v1/schedules/1/chat/1").header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN))
                // then
                .andExpect(status().isOk());
    }
    ...
    public void 스케줄_채팅_삭제_권한없음() throws Exception {
        // given
        // when
        mvc.perform(delete("/v1/schedules/1/chat/1").header(HttpHeaders.AUTHORIZATION, ACCESS_TOKEN_USER))
                // then
                .andExpect(status().isForbidden());
    }
}
