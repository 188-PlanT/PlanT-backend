...
public interface UserWorkspaceRepository extends JpaRepository<UserWorkspace, Long> {
    ...
    @Query(value = "select uw from UserWorkspace uw join fetch uw.user u join fetch uw.workspace w join fetch w.profile p where uw.user =:user")
    public List<UserWorkspace> searchByUser(User user);
	
	@Query(value = "select uw from UserWorkspace uw " + 
